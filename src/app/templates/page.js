...
import { Plus, FileText, MoreVertical, Edit, Trash2, X, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function TemplatesPage() {
  ...
  const confirmDelete = async () => {
      ...
      try {
        const response = await fetch(`/v1/templates/${id}`, { method: 'DELETE' });
        
        if (response.status === 204) {
             setSavedTemplates(prev => prev.filter(t => t.template_id !== id));
             setApiResponse({ type: 'success', message: "Template deleted successfully" });
        } else {
            const resText = await response.text();
            let message = response.ok ? "Template deleted successfully" : "Failed to delete template";
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto min-h-screen relative" onClick={() => setOpenDropdownId(null)}>
       {/* API Response Toast */}
       {apiResponse && (
         <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 animate-in fade-in slide-in-from-top-10 duration-300 ${apiResponse.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
            ...
            <button onClick={() => setApiResponse(null)} className="ml-2 hover:opacity-75"><X size={16} /></button>
         </div>
       )}
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          {savedTemplates.map(template => (
              ...
              >
                  ...
                  <div className="p-4">
                      <h3 className="font-semibold text-slate-800 mb-1 truncate">{template.template_name}</h3>
                  ...
                  <div className="absolute top-3 right-3">
                      <button 
                        onClick={(e) => {
                            e.stopPropagation();
                            setOpenDropdownId(openDropdownId === template.template_id ? null : template.template_id);
                        }}
